...
def process_text(data_structure):
    ...
    for item in data_structure:
        img = item['image']
        str = item['text'] = pytesseract.image_to_string(img).replace('-\n','')
        #boxes = item['boxes']
    return data_structure
...
def print_text(data_structure):
    for item in data_structure:
        print(item['text'])


def show_faces(data_structure):
    for item in data_structure:
        img = item['image']
        for x,y,w,h in item['face_boxes']:
            face = img.crop(x,y,x+w,y+h).resize(110,110)
            face.show()

def print_to_contact_sheet(data_structure):
        ...
        if test_word in item['text']:
            ...
            for box in item['face_boxes']:
                ...
                for x,y,w,h in box:
                    ...
                    face = img.crop((x,y,x+w,y+h)).resize((110,110)).thumbnail((110,110))
...
def main():
    ...
    #print_text(data_structure)
    show_faces(data_structure)
