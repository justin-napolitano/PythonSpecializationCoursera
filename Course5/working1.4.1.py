...
def calculate_line_height(data_structure):
    ...
    for item in data_structure:
        img = item['image']
        item['boxes'] = pageseg.segment(img.convert('1'))['boxes']
    ...
    # lets change it to the nearest full pixel by making it an integer
        item['line_height'] = int(height_accumulator/len(item['boxes']))
...
def draw_sep(img,location, gap_box):
...
def main():
    ...
    data_structure[0]['image'].show()

    print('running process_text')
