...
#        for w in VOWELS:
#            vowel_lst.append(w)
#        if self.getPossibleLetters() == vowel_lst:
#            return "pass"
#        if self.smartCoinFlip() is True:
#                frequent.lst.append(l)
#            the_most_f=frequent_lst[-1]
#            return the_most_f
#        if self.smartCoinFlip() is False:
...
#            return rand_letter
   ...
   def getMove(self,category, obscuredPhrase, guessed):
       lett = self.getPossibleLetters(guessed)
       if lett == []:
           print(lett)
           return 'pass'
       mov = self.smartCoinFlip()
       if self.smartCoinFlip():
           so = self.SORTED_FREQUENCIES[-1]
           print(so)
           return so

       elif not self.smartCoinFlip():
           print(mov)
           letterss = [letter for letter in self.SORTED_FREQUENCIES]
           print(letterss)
           ra = random.choice(letterss)
           print(ra)
           return ra
